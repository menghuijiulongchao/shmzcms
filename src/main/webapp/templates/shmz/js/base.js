	...
	$(".acver .acverR .acverQueryBox .acverLabel a")
			.click(
					function() {
						...
						var n = $(".acver .acverR .acverQueryBox .acverLabel a")
								.index(this);
						$('.acver .acverR .acverFormBox')
								.css('display', 'none');
						$('.acver .acverR .acverFormBox').eq(n).css('display',
								'block');
					})
	// 数字处理
...
function Num_Handle(NumValue) {
	...
	for (var i = 0; i < numValue.length; i++) {
		text_html += "<li style='background:#fff;color: #5e96c9;font-size:24px;float:left;width:30px;height:38px;line-height:38px;text-align: center;	margin-left:1px;'>"
				+ numValue[i] + "</li>"
	}
	text_html += "</ul>"
...
// tab切换效果
function tabChange(tag, tagCon, currentClass) {// tag
												// 用于切换的标签，tagCon用于切换的内容，currentClass更换的样式值
	$(tag).hide();
	$(tag).eq(0).show();
